...
class RepoAnalyzer:
    ...
    def get_relevant_py_files(self) -> Generator[Path, None, None]:
        ...
        for f in self.repo_path.rglob("*.py"):
            ...
            log.debug("Found relevant Python file", file=str(f))
            yield f
    ...
    def get_network_related_files(self, files: List[Path]) -> Generator[Path, None, None]:
        ...
        for py_f in files:
            ...
            if any(re.search(pattern, content) for pattern in self.compiled_patterns):
                log.debug("Network-related file detected", file=str(py_f))
                yield py_f
...
class JSRepoAnalyzer:
    ...
    def _is_valid_file(self, file: Path) -> bool:
        ...
        if any(exclusion in file_str for exclusion in self.to_exclude):
            return False
        if any(fn in file.name.lower() for fn in self.file_names_to_exclude):
            return False
        return True
